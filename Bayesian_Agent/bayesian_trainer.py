...
class bayesian_trainer(Agent_Bucket):
	...
	def play(self,can_check = False,can_raise = True, pot=None):
		...
		proba = [0,0,0]
		if bucket == 0:
			proba = [0.45,0.5,0.05]  if can_raise else [0.5,0.5,0]
		elif bucket == 1:
			proba = [0.3,0.6,0.1]  if can_raise else [0.35,0.65,0]
		elif bucket == 2:
			proba = [0.1,0.8,0.1]  if can_raise else [0.1,0.9,0]
		elif bucket == 3:
			proba = [0.05,0.85,0.1]  if can_raise else [0.05,0.95,0]
		elif bucket == 4:
			proba = [0,0.35,0.65]  if can_raise else [0,1,0] 
		else : 
